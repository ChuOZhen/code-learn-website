...
export default function Sidebar() {
  ...
  const currentLang = useMemo<Language>(() => {
    ...
    const langPart = parts[2]; // /chapters/[language]/[slug]
    if (langPart && isLanguage(langPart)) {
      return langPart;
    }
    // Default to C++ on home page or other pages
    const lang = LANGUAGES.find(l => pathname.includes(`/${l.key}/`));
    return lang?.key || 'cpp';
  }, [pathname]);
          ...
          <div className={`${isCollapsed ? 'hidden' : 'block'} flex-1 min-w-0`}>
            <Link href="/" className="block group" onClick={() => setIsOpen(false)}>
              <h1 className="text-lg font-bold text-foreground group-hover:text-primary transition-colors truncate">
                {currentLangInfo?.icon} {currentLangInfo?.label || 'C++'} 自学平台
              </h1>
            </Link>
